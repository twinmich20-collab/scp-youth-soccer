...
(() => {
  // Both fetch() and window.omelette.writeFile() resolve relative to this
  // file's own folder (site/), so read and write must use the same bare
  // filename — no '../' escape to the project root.
  const STATE_FILE_READ = '.image-slots.state.json';
  const STATE_FILE_WRITE = '.image-slots.state.json';
  ...
  function save() {
    ...
    if (!w) return;
    saving = true;
    Promise.resolve(w(STATE_FILE_WRITE, JSON.stringify(slots)))
      .catch(() => {})
